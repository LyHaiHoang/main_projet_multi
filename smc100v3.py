...
class SMC100(object):
  ...
  def __init__(self, smcID, port, backlash_compensation=True, silent=True, sleepfunc=None, CS="stateeeeeeeee", CD = False):
    """
  ...
  def move_absolute_mm(self, position_mm, smcid, waitStop=True):
    ...
    If waitStop is True then this method returns when the move is completed.
    """
    self.sendcmd('PA', position_mm, smc=smcid)  #PA commande pour bouger en absolute 
    if waitStop:
      # If we were previously homed, then something like PR0 will have no
      # effect and we end up waiting forever for ready from moving because
      # we never left ready from homing. This is why STATE_READY_FROM_HOMING
...
#        print("je suis state: ", state)
#        print("je suis done: ", done)
        self.commonstate = state
#        print("je suis commonstate: ", self.commonstate)
        self.commondone = done
        
        if state == "33":
           #☺ print("Mouvement fini")
            self.commonstate = "33"
...
#    print("On envoie cette commande: ", command)
